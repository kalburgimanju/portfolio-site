...
export default function SuccessPageClient() {
  ...
  useEffect(() => {
    if (!slug || !user || !book || recorded) return
    ...
    if (!already) {
      ...
      localStorage.setItem('portfolio_purchases', JSON.stringify(purchases))
    }

    setRecorded(true)
  }, [slug, user, book, recorded])

  if (!book || !book.driveLink) {
    return (
      <main className="min-h-screen flex items-center justify-center">
        <div className="text-slate-600">Book not found or download not available.</div>
      </main>
    )
  }
